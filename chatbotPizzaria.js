...
let cars;
try {
    const carsData = fs.readFileSync('carros.json', 'utf-8');
    cars = JSON.parse(carsData)['cars'] || [];
    console.log('✅ Carros carregados com sucesso do carros.json.');
} catch (err) {
    console.error('❌ Erro ao carregar carros.json:', err);
    cars = [];
    fs.writeFileSync('carros.json', JSON.stringify({ cars: [] }, null, 2));
    console.log('⚠️ Arquivo carros.json criado como vazio.');
}
...
function saveCars() {
    fs.writeFileSync('carros.json', JSON.stringify({ cars: cars }, null, 2));
    console.log('✅ Carros salvos em carros.json.');
}
...
async function enviarImagensDoCarro(chatId, car) {
    if (!car.images || !Array.isArray(car.images)) {
        console.log(`ℹ️ Nenhuma imagem disponível para o carro ${car.name}`);
        return;
    }
    const maxImages = 1; // Reduced to 1 to minimize resource usage
    for (let i = 0; i < Math.min(car.images.length, maxImages); i++) {
        const imagemPath = car.images[i];
        try {
            const fullPath = path.join(__dirname, 'cars', imagemPath);
...
async function enviarRelatorioParaContatos(chatId) {
    ...
    // Usar contatos fixos (você pode ajustar isso conforme necessário)
    const contacts = [
        { name: "Contato1", number: "+5511999999999" },
        { name: "Contato2", number: "+5511888888888" }
    ];
    const sendPromises = contacts.map(async (contato) => {
        let numeroWhatsApp = contato.number.replace(/\D/g, '') + '@c.us';
        try {
            ...
            if (dados.troca?.foto) {
                ...
                if (fs.existsSync(fotoPath)) {
                    ...
                    console.log(`✅ Foto de troca enviada para ${contato.name}`);
                } else {
            ...
            if (dados.venda?.foto) {
                ...
                if (fs.existsSync(fotoPath)) {
                    ...
                    console.log(`✅ Foto de venda enviada para ${contato.name}`);
                } else {
                    console.error(`❌ Foto de venda não encontrada: ${fotoPath}`);
                }
            }

            console.log(`✅ Relatório enviado para ${contato.name}`);
        } catch (err) {
            console.error(`❌ Erro ao enviar relatório para ${contato.name}:`, err);
        }
    });
...
async function enviarListaCarros(chatId) {
    ...
    for (const car of cars) {
        lista += `🚘 *${car.name}* - ${car.price || 'Preço não informado'}\n`;
    }
...
client.on('message', async msg => {
    ...
    if (estado?.etapa === 'aguardandoEstadoVenda') {
        if (isValidCondition(textoOriginal)) {
            ...
            await client.sendMessage(chatId, `Estado registrado: ${textoOriginal}. Informe o *preco desejado* para a venda (ex.: 50000):`);
        } else {
    ...
    if (estado?.etapa === 'aguardandoSelecaoCarro') {
        const carFound = cars.find(car => texto.includes(car.name.toUpperCase()));
        if (carFound) {
            const dados = interessesClientes.get(chatId) || { interesses: [] };
            dados.interesses.push(carFound.name);
            interessesClientes.set(chatId, dados);

            await client.sendMessage(chatId, `🔍 Aqui estão os detalhes do carro *${carFound.name}*:\n\n🗓 Ano: ${carFound.year || 'sem ano'}\n💲 Preço: ${carFound.price || 'Preço não informado'}\nDescrição: ${carFound.description || 'Sem descrição'}`);
            await enviarImagensDoCarro(chatId, carFound);
            await wait(500);
            await client.sendMessage(chatId, `Deseja simular um financiamento deste carro?\nDigite "SIMULAR", ou envie outro nome de carro.`);
            estadoCliente.set(chatId, { etapa: 'aguardandoFinanciamento', carroEscolhido: carFound.name });
    ...
    if (estado?.etapa === 'menuInicial' || estado?.etapa === 'aguardandoFinanciamento') {
        ...
        if (carFound) {
            let dados = interessesClientes.get(chatId) || { interesses: [] };
            if (!dados.financiamento) dados.financiamento = {};
            dados.financiamento.carroEscolhido = carFound.name;
            interessesClientes.set(chatId, dados);

            estadoCliente.set(chatId, { etapa: 'aguardandoEntrada', carroEscolhido: carFound.name });
            await client.sendMessage(chatId, `Ótimo, você escolheu o *${carFound.name}*. Agora, informe o valor da *entrada* disponível (R$):`);
        } else {
